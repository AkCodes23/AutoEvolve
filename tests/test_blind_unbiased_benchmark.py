...
class TestBlindUnbiasedBenchmark:
    ...
    def test_blind_unbiased_evaluation_run(self):
        ...
        assert len(ranks) == 7
        # Top rank must be AutoEvolve v4.0
        assert ranks[0]["condition_id"] == "c7_swarm_v40"
        assert summary["c7_swarm_v40"] > summary["c6_lats_prm_v35"] > summary["c5_praxist_v3"] > summary["c0_baseline"]
        # Statistically significant margin over v3.0 baseline
        assert summary["c7_swarm_v40"] - summary["c5_praxist_v3"] >= 10.0
