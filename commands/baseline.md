...
1. **Name the signal.** In one line, state how a change will be judged better: a specific
...
2. **Find or build the cheapest scorer.** Locate the repo's existing test/benchmark/lint
...
3. **Keep the ruler separate.** The scorer must live outside the code it measures and stay
   effectively read-only — so the code being optimized can never quietly rewrite its own
   grader.
4. **Record the baseline.** Ensure a clean git tree (commit or stash), run the scorer, and
   write down the starting number(s). HEAD is now your best-known state.
5. **Report** the signal and the baseline, and confirm they look right before iterating.
