...
1. **Check for `DIRECTION.md`.** If the repo has this human-owned file, read it and treat
   ...
   Signal:     <how "better" is judged; where the read-only scorer/test lives>
   Guardrails: <what must never break: correctness, safety, explicit requirements>
   Budget:     <stop condition: time, number of experiments, or "until the signal is met">
   ```
