...
def cmd_hooks(target: str, force: bool = False) -> int:
    ...
    # Where this copy of AutoEvolve lives, so the hook can find the mechanisms from inside a
    # repository that does not contain them. Every check used to be guarded by `[ -f
    # "scripts/check.py" ]` relative to the target, which is true only in AutoEvolve's own
    # repository. Anywhere else all three guards failed, nothing ran, and the hook printed
    # "Checks passed", which is the exact reassurance it exists to deny. Forward slashes because
    # the hook runs under sh even on Windows.
    home = os.path.dirname(os.path.abspath(__file__)).replace("\\", "/")
    script_content = (
        "#!/bin/sh\n"
        "# AutoEvolve pre-commit hook.\n"
        ...
        "if [ -z \"$PY\" ]; then\n"
        "    echo '[autoevolve pre-commit] no python found on PATH; NO CHECKS RAN' >&2\n"
        "    exit 0\n"
        "fi\n"
        f"AUTOEVOLVE_HOME=\"{home}\"\n"
        "echo '[autoevolve pre-commit] Running checks...'\n"
        "ran=0\n"
        "# check.py and build_adapters.py verify AutoEvolve's own invariants, so they are run\n"
        "# only when the repository being committed to is AutoEvolve itself. Pointing them at\n"
        "# someone else's project would fail on files it has no reason to have.\n"
        "if [ -f \"scripts/check.py\" ] && [ -f \"AGENTS.md\" ] && [ -d \"adapters\" ]; then\n"
        ...
        "    COMMENTS=\"scripts/comments.py\"\n"
        "elif [ -f \"$AUTOEVOLVE_HOME/scripts/comments.py\" ]; then\n"
        "    COMMENTS=\"$AUTOEVOLVE_HOME/scripts/comments.py\"\n"
        "fi\n"
        "if [ -n \"$COMMENTS\" ]; then\n"
        "    \"$PY\" \"$COMMENTS\" --staged --strict || exit 1\n"
        "    ran=$((ran+1))\n"
        "fi\n"
        "# Never announce a pass that covered nothing. A hook that says the commit was checked\n"
        "# when it was not is worse than no hook: it is the reason the author stopped looking.\n"
        "if [ \"$ran\" -eq 0 ]; then\n"
        "    echo '[autoevolve pre-commit] NO CHECKS RAN: could not find the AutoEvolve scripts.' >&2\n"
        f"    echo '[autoevolve pre-commit] Looked in ./scripts and {home}/scripts.' >&2\n"
        "    echo '[autoevolve pre-commit] Re-run: python3 autoevolve.py hooks --target . --force' >&2\n"
        "    exit 0\n"
        "fi\n"
        "echo \"[autoevolve pre-commit] $ran check(s) passed.\"\n"
        "exit 0\n"
    )
